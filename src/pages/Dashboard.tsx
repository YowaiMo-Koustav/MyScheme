...
const Dashboard = () => {
  ...
  const saved = useMemo(() => allSchemes.filter(s => savedSchemes.includes(s.id)), [savedSchemes]);

  return (
    <div className="min-h-screen bg-background">
      <div className="bg-navy py-8 pb-12">
        <div className="container mx-auto px-4 max-w-5xl">
          <Button variant="ghost" className="text-white/70 hover:text-white mb-4" onClick={() => navigate('/')}>
            <ArrowLeft className="w-4 h-4 mr-1" /> {t('होम', 'Home')}
          </Button>
          <h1 className="text-3xl md:text-4xl font-display font-bold text-white">{t('मेरा डैशबोर्ड', 'My Dashboard')}</h1>
          <p className="text-white/60 mt-2 font-body">{t('आपकी सहेजी गई योजनाएं', 'Your saved schemes and profile summary')}</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            <div className="rounded-2xl bg-card border border-border p-6">
              <h2 className="font-display font-bold text-foreground text-xl mb-4">
                {t(`सहेजी गई योजनाएं (${saved.length})`, `Saved Schemes (${saved.length})`)}
              </h2>
              {saved.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  <Bookmark className="w-8 h-8 mx-auto mb-3 opacity-40" />
                  <p className="font-body">{t('अभी तक कोई योजना सहेजी नहीं गई', 'No saved schemes yet.')}</p>
                  <Button className="mt-4 bg-primary text-primary-foreground" onClick={() => navigate('/matcher')}>
                    {t('योजनाएं खोजें', 'Find Schemes')} <ArrowRight className="w-4 h-4" />
                  </Button>
                </div>
              ) : (
                <div className="space-y-3">
                  {saved.map((scheme, i) => (
                    <motion.div key={scheme.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-foreground cursor-pointer hover:text-primary transition-colors truncate" onClick={() => navigate(`/scheme/${scheme.id}`)}>{scheme.name}</h3>
          ...
          <div>
            <div className="rounded-2xl bg-card border border-border p-6 sticky top-4">
              <h3 className="font-display font-bold text-foreground mb-4">{t('आपकी प्रोफ़ाइल', 'Your Profile')}</h3>
              {Object.keys(profile).length === 0 ? (
                <p className="text-muted-foreground text-sm font-body">{t('अभी प्रोफ़ाइल नहीं है', 'No profile data yet.')}</p>
              ) : (
                  ...
                  {profile.selected_roles && (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {profile.selected_roles.map(r => <Badge key={r} variant="secondary" className="text-xs capitalize">{r.replace('_', ' ')}</Badge>)}
                      </div>
                    </div>
                  )}
              ...
              <div className="mt-6 space-y-2">
                <Button className="w-full bg-primary text-primary-foreground" onClick={() => navigate('/matcher')}>
                  <Search className="w-4 h-4 mr-2" /> {t('और योजनाएं खोजें', 'Find More Schemes')}
                </Button>
                <Button variant="outline" className="w-full" onClick={() => navigate('/results')}>{t('परिणाम देखें', 'View Results')}</Button>
              </div>
            </div>
          </div>
        </div>
